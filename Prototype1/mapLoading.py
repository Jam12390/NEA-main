...
def loadMapData(
    ...
    for row in segmentedData:
        ...
        for column in row:
            if (
                not int(column) in INVALIDKEYS
            ):  # int(column) == -1 and not int(column) == STARTKEY and not int(column) == ITEMKEY and not int(column) == ENEMYKEY: #if tile not empty
                try:
                ...
                uWall = segmentedData[max(0, currentNodePosition[0] - 1)][
                    currentNodePosition[1]
                ]
                dWall = segmentedData[
                    min(len(segmentedData) - 1, currentNodePosition[0] + 1)
                ][currentNodePosition[1]]
                lWallPresent = not (
                    int(lWall) in INVALIDKEYS or currentNodePosition[1] - 1 < 0
                )
                rWallPresent = not (
                    int(rWall) in INVALIDKEYS or currentNodePosition[1] + 1 >= len(row)
                )
                ...
                lCorner = (
                    floorPresent
                    and rWallPresent
                    and (not lWallPresent)
                    and (not roofPresent)
                )
                rCorner = (
                    floorPresent
                    and lWallPresent
                    and not rWallPresent
                    and not roofPresent
                )
