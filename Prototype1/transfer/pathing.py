    ...
    if useDirections and directionalGraph != None:
        ...
        for nodeIndex in range(0, len(presence)):
            if presence[nodeIndex][
                0
            ]:  # if coord exists in the graph and is reachable by an optional directionalGraph
...
def getTopDownPath(
    ...
    while end != currentNode.coord and not end in currentNode.nextNodes:
        if preferDirection == None:
            ...
            currentNodeIndex = getNextNodeToVisit(
        ...
        for node in adjacentNodes:
            ...
            if index == -1:
                ...
                if (
                    newDistance < nodes[index].shortestDistance
                    and abs(newDistance - nodes[index].shortestDistance) > tolerance
                ):
                    nodes[index].shortestDistance = newDistance
                    overriddenPreviousNodeIndex = getNodeFromCoord(
                    ...
                    if (
                        ...
                        nodes[overriddenPreviousNodeIndex].nextNodes.remove(
                            nodes[index].coord
                        )
                    nodes = cascadeUpdate(nodes=nodes, startNode=nodes[index])
...
def canFallTowardsPoint(
    ...
    fallNodes = list[precompile.Point](
        precompile.getPointsAcrossCurve(
            u=0,
            g=gravity,
            origin=origin,
            nodeMap=nodeMap,
...
def findFreeNode(nodeMap, start: tuple[int, int]):  # (y, x)
    ...
    if nodeMap[clamp(start[0], 0, len(nodeMap) - 1)][
...
def main(
    ...
    path = pathfind(
        ...
        start=(int(start[0]), int(start[1])),
            ...
            int(end[0]),
            int(end[1]),
        ),  # check for if path wraps around vertically using obj.previousNode and compare if the current previous node is on the same or lower y axis and that "d" is preferred as a direction
        tolerance=0,
        waypoints=waypoints,
