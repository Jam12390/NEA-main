...
class Entity(PhysicsObject):
    ...
    def addEffect(self, ID: int):
        ...
        while not freeInstance:
            if not str(ID) + "-" + str(currInstance) in self._effects.keys():
                ...
                currInstance += 1
        self._effects[str(ID) + "-" + str(currInstance)] = allEffects[
            ID
        ]  # allEffects is an imported dictionary containing all effects in the game in the format: array[2]
    ...
    def _recalculateAttributes(self):
        ...
        self._velocityCap.x = max(
            self._velocityCap.x, self._velocityCap.x * self._speed
        )  # increase speed cap by a factor of _speed
        self._velocityCap.y = max(
            self._velocityCap.y, self._velocityCap.y * self._speed
        )
    ...
    def modifyStat(self, stat: str, operator: str, magnitude: float):
        ...
        match stat:
            ...
            case "cooldown":
                self.attackCooldown = operators[operator](
                    self.attackCooldown, magnitude
                )
    ...
    def path(
        ...
        if len(self.currentPath) > 0:
            ...
            if self.currentNode == self.currentPath[0] and self.isGrounded:
                ...
                # // by adding a time limit of 0.5s between nodes.
                # // If the entity hasn't moved nodes by the time 0.5s is up,
                # // delete the first index of currentPath and continue
                self.framesSinceLastNode = 0
                self.previousPathCoord = self.currentPath[0]
                self.currentPath.pop(0)
                self.removeForce(axis="x", ref="xPathing")
    ...
    def update(
        self, collidableObjects, precompiledData, nodeMap, nodeSep, pathingTo
    ):  # playerMoved=(0, 0)
        for key in self._effects.keys():
