...
class CompiledWaypointResponse:
    def __init__(
        self,
        waypoints: list[tuple[tuple[int, int], str, tuple[int, int]]],
        disconnectedWaypoints: tuple[int, int],
    ):
        self.WAYPOINTS = waypoints
...
# Checks if a coordinate is in a list of Point objects
...
def getPointsAcrossCurve(
    ...
    if not solveForMax:  # Default logic
        ...
        while not hitHash:  # The function stops when it hits a wall
            ...
            currentPoint = Point(
                x=coord[1], y=coord[0], nodeMap=nodeMap
            )  # And convert it into a Point object
            if (
                currentPoint.isEmpty() and currentPoint.isValid()
            ):  # So we can use the inbuild isEmpty and isValid methods
        ...
        solvePastMax
    ):  # From maxima onwards - // Used for fallTowardsPoint() in pathing.py
    ...
    for pointIndex in range(0, len(uniquePoints)):
        uniquePoints[pointIndex] = (
            Point(  # Converting each unique point to a Point object
                x=uniquePoints[pointIndex][1],  # uniquePoints[pointIndex] => [y, x]
                y=uniquePoints[pointIndex][0],
                nodeMap=nodeMap,
            )
        )
    ...
    for currentNode in parabolaPoints:
        ...
        if not (hitRoof or hitWall or hitFloor) and currentNode.isValid():
            ...
            if not currentNode.isEmpty() and lowerNode.isEmpty() and yVelocity >= 0:
                ...
                roofNode = Point(
                    x=lowerNode.x(), y=lowerNode.y(), nodeMap=nodeMap
                )  # Mark the lowerNode as the reversal point for the parabola
...
def getLowerNodes(
    ...
    while len(topNodes) != 0:
        ...
        for node in topNodes:
            ...
            if not (
                inList(
                    query=currentNode, ls=topNodes
                )  # // Making sure not to start overlapping points
                or inList(
                    query=currentNode, ls=foundNodes
                )  # // and marking new topNodes which are already there
            ):
                    ...
                    if distanceFromTopNode % 2 == 0:  # if distanceFromTopNode % n == 0
                        for x in range(0, 2):  # Do this for both left and right
                            ...
                            if (
                                ...
                                and not foundNewTopNode[
                                    max(0, xStep[x])
                                ]  # And we haven't marked a new topNode for this column yet
                            ):
                                newTopNodes.append(
                                    Point(
                                        x=potentialNode.x(),
                                ...
                                not potentialNode.isEmpty()
                            ):  # If the node across is a wall
                                foundNewTopNode[max(0, xStep[x])] = (
                                    False  # Reset foundNewTopNode for this direction
                                )
                            xStep *= 1
                ...
                if (
                    not currentNode.isEmpty()
                ):  # If at any point the lower node is a wall
                    currentNode.setY(newY=currentNode.y() - 1)
                    floorNodes.append(currentNode)  # Mark the upper node as a floorNode
        topNodes = list(tuple(newTopNodes))

    return LowerNodesResponse(nodes=foundNodes, floorNodes=floorNodes)
...
def traverseFloor(
    ...
    for x in range(2):
        ...
        while (
            current.isValid() and not stop
        ):  # Until the current node is invalid or we've stopped
            ...
            if current.isEmpty():
                ...
                while (  # While the current node is within the graph's bounds, is empty and is within the entity's jump range
                    ...
                    currentCollisionStates = [
                        leftNode.isValid()
                        and not leftNode.isEmpty(),  # => if [direction] node is within the graph's bounds and isn't a wall
                        rightNode.isValid() and not rightNode.isEmpty(),
                    ]
                    if (
                        previousCollisionStates[0] and not currentCollisionStates[0]
                    ):  # Checks if collision has changed from true to false on the left side
                        newFloors.append(
                            leftNode
                        )  # In which case a new floor has been found
                        waypoints.append(
                    ...
                    if (
                        previousCollisionStates[1] and not currentCollisionStates[1]
                    ...
                    previousCollisionStates = list(
                        tuple(currentCollisionStates)
                    )  # Move currentCollisionState to previousCollisionState
                    currentCollisionStates = [
                        False,
                        False,
                    ]  # And reset currentCollisionState
        ...
        current = Point(
            x=origin.x(), y=origin.y(), nodeMap=nodeMap
        )  # And reset to origin
        next = Point(x=origin.x(), y=origin.y(), nodeMap=nodeMap)

    return FloorResponse(
        nodes=foundNodes, corners=corners, newFloors=newFloors, waypoints=waypoints
    )
...
def removeDuplicateWaypoints(waypoints: list):
    ...
    while len(waypoints) != 0:
        ...
        if (
            ...
            "->",
            waypoint[0],
        ) in waypoints:  # // Check if another waypoint stores (y2, x2) "->" (y1, x1)
            waypoints.remove((waypoint[2], "->", waypoint[0]))
            waypoint = (
                waypoint[0],
                "<->",
                waypoint[2],
            )  # // Since we can compress it into (y1, x1) "<->" (y2, x2) if so
        if (
            not waypoint[0] == waypoint[2] and not waypoint in cleanWaypoints
        ):  # Presence check to ensure we don't add a duplicate to our clean list
...
def connectAdjacentWaypoints(
    ...
    waypointGroups = (
        []
    )  # 2D list of coordinates on the same y level which appear in waypoints
    while (
        len(disconnectedWaypoints) != 0
    ):  # // Condition controlled loop since we change disconnectedWaypoints within the loop
        newGroup = []
        ...
        for point in disconnectedWaypoints:
            if point[0] == yQuery:
                newGroup.append(
                    point
                )  # // Removes the point from disconnectedWaypoints
            else:
                preservedWaypoints.append(
                    point
                )  # // since disconnectedWaypoints becomes preservedWaypoints after this loop
    ...
    for group in waypointGroups:
        ignoreNextConditions = (
            False  # Guarantees that the next node is a corner if true
        )
        if not group == []:
            cornerIndexes = list[int](
                [0]
            )  # The first index will always be a corner // since it's sorted by ascending x
            for index in range(0, len(group) - 1):
                ...
                if (
                    group[index][1] == group[index + 1][1] - 1
                ):  # If the current waypoint is adjacent to the next waypoint
                    waypoints.append((group[index], "<->", group[index + 1]))
                elif not attemptGroundTraversal(
                    start=group[index],
                    end=group[index + 1],
                    nodeMap=nodeMap,  # Checks if A can't reach B without jumping
                ):
                    cornerIndexes.append(max(1, index))  # If so, mark it as a corner
                    ignoreNextConditions = (
                        True  # // After a corner, the next node will always be a corner
                    )
                else:
                    waypoints.append(
                        (group[index], "<->", group[index + 1])
                    )  # // If A can reach B, mark it as a bidirectional waypoint
...
def precompileGraph(
    ...
    while len(floors) != 0:  # Loops until no new floors are found through the loop
        ...
        for floor in floors:
            if (
                not floor.getCoord() in traversedFloors
            ):  # Verifies that the floor hasn't been traversed yet
                ...
                for newFloor in floorData.NEWFLOORS:
                    if (
                        ...
                        newFloors.append(
                            newFloor
                        )  # Will become floors at the end of the while loop
                ...
                for (
                    ...
                    floorData.NODES
                ):  # Can be shortened but it harms readability too much
        ...
        for corner in corners:  # corner => (Point, direction)
            ...
            for x in range(
                ...
                if (
                    x == 0
                ):  # and an extra variable "observedList" to track which list is in use
                ...
                for node in observedList:
                    if node.data != " ":
                        indexesToRemove.append(
                            observedList.index(node)
                        )  # By removing wall nodes
                preservedNodes = []
            ...
            for newFloor in columnNodeData["floorNodes"]:
                if not newFloor.getCoord() in traversedFloors and not (
                    ...
                    or inList(
                        query=newFloor, ls=floors
                    )  # Prevents duplicates in floors
                ):
